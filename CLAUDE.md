...
1. **BT gamepad (8BitDo) still untested on hardware that's actually BLE-capable** — three physical controllers tried (8BitDo SF30 Pro, Xbox One S, 8BitDo Zero 2), none ever detected as a BLE-advertising HID device by our scanner or by a dedicated BLE scanner app (LightBlue) — see Resolved/dead-end notes below. Paused: no controller currently on hand has *confirmed* (not inferred) BLE support. The Switch-mode parsing code itself remains unverified against real hardware; first 200 raw reports are still logged to Serial to allow byte-offset verification whenever a confirmed BLE-capable controller (8BitDo Pro 2, Ultimate, or SN30 Pro+, per manufacturer spec — not just "works with Switch") becomes available.
...
- **Three controllers tried for "8BitDo untested on hardware" — none confirmed BLE, item paused (2026-06-19)**:
  - **8BitDo SF30 Pro**: same hardware generation as the original SN30 Pro (non-Plus), which the existing docs already noted is Bluetooth-Classic-only. Confirmed via web research (bluepad32 project changelog lists it among BR/EDR-connected devices, separately from its BLE device list) before attempting any pairing — avoided a wasted test cycle.
  - **Xbox One S controller** (2016+ revision, the one with genuine BLE support per Microsoft/community documentation): paired via the sync button (not the Xbox button) multiple times with Serial Monitor watching; never once appeared in the scan — not by name, not by HID service UUID, and not by Microsoft's BT-SIG manufacturer ID (`0x0006`, checked via a temporary diagnostic added then reverted in `ble_gamepad.cpp`). Independently confirmed with a dedicated BLE scanner app (LightBlue) — the controller never advertises anything over BLE during sync mode, full stop. This specific unit's hardware/firmware revision does not do BLE advertising despite the general "Xbox One S has BLE" claim for the product line — that capability varied across production batches.
  - **8BitDo Zero 2**: pairing combo differs from other 8BitDo models — Switch/BLE mode is **Y+Start to power on, then hold Select** (not X+Start, which is Windows/XInput mode on this model). Followed that sequence correctly (confirmed by the controller later identifying as "Pro Controller" when paired through Android's system Bluetooth settings — that's the name 8BitDo controllers use to impersonate a Switch Pro Controller). Even so, never appeared in our scanner or LightBlue across multiple attempts, including after unpairing from a phone that may have been intercepting it via prior bonding (same interference pattern suspected with the Xbox/Mac case). Likely explanation: Android's system "Pair new device" screen primarily discovers **Classic Bluetooth**, not BLE — so Android finding it instantly as "Pro Controller" does not actually confirm BLE was used for that specific connection. Many modern 8BitDo controllers are dual-mode (Classic + BLE) chips; this unit may simply not be advertising over BLE in practice regardless of which pairing combo is used.
  - **Conclusion:** the 8BitDo Switch-mode `parseReport()` code remains coded but unverified against real hardware. Paused — testing further controllers blind isn't productive; the next candidate needs *manufacturer-confirmed* BLE support (not inferred from "works with Switch"), e.g. checking 8BitDo's own spec page for the Pro 2 or Ultimate before acquiring one.

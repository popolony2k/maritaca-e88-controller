...
4. **Dr.One re-arms when B is pressed after a stick-landing (pending — needs PCAP capture)** — `TakeOff` (0x01) is a toggle, not absolute. If the drone auto-disarms itself on touchdown (its own ground sensor, no command from us) while `FlightController` is still in `Flying`, pressing B sends the toggle expecting to land — but since the drone is already disarmed, the toggle flips it to armed+takeoff instead, causing an unwanted re-launch. A throttle-based "assume landed" heuristic was considered and rejected: throttle alone can't distinguish "on the ground" from "slow controlled descent" or "paused at neutral while hovering" — a false positive would silently switch to Idle mid-flight and ignore real stick input. Needs a fresh PCAP capture (user to provide) of the KY UFO app reproducing this scenario, to determine whether the drone sends telemetry/status feedback that explains how the app avoids this (real fix), or whether the app uses the same unreliable heuristic (no protocol-level fix available). Practical workaround until then: always press B to land deliberately before the drone's own sensor would auto-disarm — the explicit B→Landing→Idle flow is already confirmed working. If the drone self-lands first, don't press B — pick it up by hand.
